...
#
# Author     : Arthik kumar
# Tool       : ReconXplorer v1.2
# Usage      : python3 ReconXplorer.py example.com
...
# ReconXplorer Help Context
def helper():
        print(bcolors.OKBLUE+"Information:"+bcolors.ENDC)
        print("------------")
        print("\t./ReconXplorer.py example.com: Scans the domain example.com.")
        print("\t./ReconXplorer.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
        print("\t./ReconXplorer.py example.com --nospinner: Disable the idle loader/spinner.")
        print("\t./ReconXplorer.py --update   : Updates the scanner to the latest version.")
        print("\t./ReconXplorer.py --help     : Displays this help context.")
        print(bcolors.OKBLUE+"Interactive:"+bcolors.ENDC)
        print("------------")
        print("\tCtrl+C: Skips current test.")
        print("\tCtrl+Z: Quits ReconXplorer.")
...
def logo():
    ...
    logo_ascii = """
                                  __   _  ___    @ 
                                 /__\ |_/  | |__|||_/"""+bcolors.BADFAIL+" ●"+bcolors.WARNING+"""_/( | |_/ _ _
                                /    \| \  | |  ||| \                                               | | \
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"ReconXplorer received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
            sys.exit(1)
...
tool_cmd   = [
                ...
                #2
                ["wget -O /tmp/ReconXplorer_temp_aspnet_config_err --tries=1 ","/%7C~.aspx"],

                #3
                ["wget -O /tmp/ReconXplorer_temp_wp_check --tries=1 ","/wp-admin"],

                #4
                ["wget -O /tmp/ReconXplorer_temp_drp_check --tries=1 ","/user"],

                #5
                ["wget -O /tmp/ReconXplorer_temp_joom_check --tries=1 ","/administrator"],
                ...
                #75
                ["wget -O /tmp/ReconXplorer_temp_aspnet_elmah_axd --tries=1 ","/elmah.axd"],
                ...
                #78
                ["wapiti "," -f txt -o ReconXplorer_temp_wapiti"],
...
if args_namespace.help or (not args_namespace.update \
    ...
    logo()
    print("ReconXplorer is updating....Please wait.\n")
    spinner.start()
    ...
    if rs_internet_availability == 0:
        ...
        spinner.stop()
        sys.exit(1)
    cmd = 'sha1sum ReconXplorer.py | grep .... | cut -c 1-40'
    oldversion_hash = subprocess.check_output(cmd, shell=True)
    oldversion_hash = oldversion_hash.strip()
    os.system('wget -N https://raw.githubusercontent.com/skavngr/ReconXplorer/master/ReconXplorer.py -O ReconXplorer.py > /dev/null 2>&1')
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print("\t"+ bcolors.OKBLUE +"You already have the latest version of ReconXplorer." + bcolors.ENDC)
    else:
        clear()
        print("\t"+ bcolors.OKGREEN +"ReconXplorer successfully updated to the latest version." +bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    os.system('rm /tmp/ReconXplorer* > /dev/null 2>&1') # Clearing previous scan files
    os.system('clear')
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t"+bcolors.BG_ERR_TXT+"ReconXplorer was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print("\t"+bcolors.OKGREEN+"All Scanning Tools are available. Complete vulnerability checks will be performed by ReconXplorer."+bcolors.ENDC)
    else:
        print("\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. ReconXplorer will still perform the rest of the tests. Install these tools to fully utilize the functionality of ReconXplorer."+bcolors.ENDC)
    ...
    while(tool < len(tool_names)):
        ...
        temp_file = "/tmp/ReconXplorer_temp_"+tool_names[tool][arg1]
        ...
        if runTest == 1:
                ...
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit ReconXplorer.\n" + bcolors.ENDC)
                rs_skipped_checks = rs_skipped_checks + 1
    ...
    if len(rs_vul_list)==0:
        ...
        with open(vulreport, "a") as report:
            while(rs_vul < len(rs_vul_list)):
                ...
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/ReconXplorer_temp_"+vuln_info[arg1]
            ...
            print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+vulreport+bcolors.ENDC+" is available under the same directory ReconXplorer resides.")
    ...
    for file_index, file_name in enumerate(tool_names):
        with open(debuglog, "a") as report:
            try:
                with open("/tmp/ReconXplorer_temp_"+file_name[arg1], 'r') as temp_report:
                        data = temp_report.read()
    ...
    os.system('rm /tmp/ReconXplorer_te* > /dev/null 2>&1') # Clearing previous scan files
